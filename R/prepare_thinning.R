...
#' \item BA: share of basal area removal in thinning (share or target (m²/ha))
#' \item Vol: share of volume removal in thinning (share or target (m³/ha))
#' \item crop_trees: number of crop trees (N/ha)
#' }
...
prepare_thinning <- function(
  ...
  if( is.null(thinning) ){
    ...
    thinning = array(NA_real_, dim = c(1,8,n_sp))

  } else {

    if( !identical( c("species","age","stems_n","stem","root","foliage","BA","Vol","crop_trees"), colnames(thinning) ) ){
      stop( 'Column names of the thinning table must correspond to: species, age, stems_n, stem, root, foliage, BA, Vol, crop_trees' )
    }
    ...
    thinning$crop_trees[which(thinning$crop_trees < 0)] = 0 # check crop tree bounds
    ...
    thinning = simplify2array(by(thinning[,3:10], thinning[,1], as.matrix))
  }
